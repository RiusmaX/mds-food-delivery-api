...
router.route('/login')
  .post((req, res) => {
    ...
    User.findOne({ email: email }, (error, user) => {
      ...
      // Comparer les mots de passes (celui en paramètre et celui stocké dans l'utilisateur récupéré) avec la méthode comparePassword
      user.comparePassword(password, (error, isMatch) => {
        if (error || !isMatch) return res.status(403).send('Invalid Credentials')
        // Si mot de passe correct, on génère un token et on l'envoit
